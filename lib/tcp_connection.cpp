...
int handle_write(void *data) {
    ...
    if (nwrited > 0) {
        ...
        fischer_msgx("handle_write for tcp connection %s",tcpConnection->name);
    }
...
int tcp_connection_send_data(struct tcp_connection *tcpConnection,void *data,int size) {
    ...
    if (!channel_write_event_enable(channel) && buffer_readable_size(outputBuffer) == 0) {
...
void tcp_connection_shutdown(struct tcp_connection *tcpConnection){
    if (shutdown(tcpConnection->channel->fd,SHUT_WR) < 0) {
        fischer_msgx("tcp_connection_shutdown failed,sokcet = %d",tcpConnection->channel->fd);
    }
}
